...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if($info=="groupDelete"){
    ...
    $sqlCheck=mysqli_query($conexion,"SELECT nombre,apellido,id FROM menor WHERE activo='1' AND id_grupo='$id'");
    $tupla = mysqli_fetch_array($sqlCheck);

    if(empty($tupla)){
      $sql1="DELETE FROM grupo WHERE id='$id'";
      if ($conexion->query($sql1)){
        header('Location: grupos.php');
      } else {
        $avisos["errDelete"] = "Error. falló la base de datos.<br><br>";
      }
    } else {
      $avisos["errDelete"] = "Error. Aún hay menores en este grupo.<br><br>";
      ?>
      <script>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                          <div class="header">
                            ...
                            <p class="category">Revisa la información de este grupo y cambiala si es necesario. </p>
                          </div>
                          <hr>
                          <div class="content">
                            ...
                            <form method="POST" enctype="multipart/form-data">
                              ...
                              <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label>Etapa</label>
                                        <input type="text" class="form-control" name="etapa" value="<?php echo @$etapa; ?>" required>
                                    </div>
                                </div>
                                  <div class="col-md-4">
                                      <div class="form-group">
                                        ...
                                        <select class="form-control" name="maestra" required>
                                          <option value="<?php echo $corAct; ?>" selected><?php echo $resAct; ?></option>
                                          <?php
                              ...
                              <input type="hidden" name="info" value="group"/>
                               <button type="submit" class="btn btn-info btn-fill pull-right">Actualizar Información</button>
                               <font color="red"> <?php echo @$avisos["errInfo"]?></font>
                               <font color="green"> <?php echo @$avisos["veriInfo"]?></font>
                            ...
                            <!--VER MENORES EN EL GRUPO-->
                              ...
                              <hr>
                              <form method="POST" enctype="multipart/form-data">
                                <p>Eliminar grupo de forma permanente <br>No deben de haber menores en el grupo para poder eliminarlo</p>
                                <input type="hidden" name="info" value="groupDelete"/>

                                 <font color="green"> <?php echo @$avisos["veriDelete"]?></font>
                                 <font color="red"> <?php echo @$avisos["errDelete"]?></font>
                                 <button type="submit" class="btn btn-info btn-fill pull-left">Eliminar grupo</button>
                                 <div class="clearfix"></div>
                              </form>
